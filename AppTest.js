...
class App extends React.Component {
	...
	componentDidMount() {
		...
		Fire.shared.auth.onAuthStateChanged(user => {
			// const query = Fire.shared.collection('users').doc(user.id);
			// this.unsubscribe = query.onSnapshot(querySnapshot => {
			// 	querySnapshot.docChanges().forEach(async change => {
			// 		if (change.type === 'added' || change.type === 'modified') {
			// 			await this.setState({
			// 				accessToken: change.doc.data().accessToken
			// 			});
			// 		}
			// 	});
			// });
	...
	render() {
		...
		if (this.state.loading)
			...
			return <ResetPasswordScreen actionCode={this.state.actionCode}
			clearMode={() => this.setState({ mode: null })} />
		}
